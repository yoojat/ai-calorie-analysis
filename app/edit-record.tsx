...
export default function EditRecordScreen() {
  ...
  let initialRecord: DietRecord;
  try {
    initialRecord = JSON.parse(params.record ?? "{}");
  } catch {
    initialRecord = {
      id: "",
  ...
  const [items, setItems] = useState<DietItem[]>(
    initialRecord.items.length > 0
      ? initialRecord.items
      : [{ food_name: "", calories: 0 }]
  );

  const updateItem = (index: number, field: keyof DietItem, value: string | number) => {
    const next = [...items];
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    // "저장 중..." 표시를 위해 다음 틱에서 저장 실행
    setTimeout(async () => {
      try {
        const timePart = initialRecord.recordedAt.includes("T")
          ? initialRecord.recordedAt.slice(11, 19)
          : "12:00:00";
        const updatedRecord: DietRecord = {
          ...initialRecord,
          date,
          recordedAt: `${date}T${timePart}.000Z`,
          items: validItems,
        };
        await updateDietRecord(updatedRecord);
        notifyRefreshNeeded();
        router.back();
        ...
        setSaving(false);
        Alert.alert("저장 실패", "기록 수정 중 문제가 발생했습니다.");
      }
    });
  };
          ...
          {items.map((item, index) => (
            <View key={index} style={styles.itemCard}>
              ...
              <TextInput
                ...
                onChangeText={(v) => updateItem(index, "calories", Number(v) || 0)}
