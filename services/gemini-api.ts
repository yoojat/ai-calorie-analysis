    ...
    contents: [
      {
        parts: [
          ...
          {
            text: "이 이미지 속 음식을 분석해서 칼로리와 탄단지 정보를 JSON으로 줘. 따로 제품에 대한 설명은 빼고 오직 JSON 형식으로 줘. 칼로리 정보는 calories 키에 있어. 칼로리 정보에 단위를 붙이지 말고 숫자값만 표시해줘. 음식이름 정보도 같이 표시해줘. 음식이름 정보는 name 키에 있어. 음식이름은 요청을 시도하는 국가를 기준으로 표시해줘. 여러개의 음식이 있을수도 있으니 배열로 줘. 단백질 정보는 protein 키에 있어. 탄수화물 정보는 carbohydrates 키에 있어. 지방 정보는 fat 키에 있어. 단위는 표시하지말고 숫자 값만 표시해줘. 각 음식의 수량도 표시해줘. 수량은 quantity 키에 있어. 수량의 단위는 표시하지말고 숫자 값만 표시해줘.",
          },
        ],
      },
    ],
